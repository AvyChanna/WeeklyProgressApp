package com.example.weeklyprogressapp.chart;
...
import com.example.weeklyprogressapp.R;
...
public class RoundedBarChart extends BarChart {
    ...
    private class RoundedBarChartRenderer extends BarChartRenderer {
        ...
        RoundedBarChartRenderer(BarDataProvider chart, ChartAnimator animator, ViewPortHandler viewPortHandler, int mRadius) {
            ...
            cornerRadiusArr = new float[]{mRadius, mRadius, mRadius, mRadius, 0, 0, 0, 0};
        }
        ...
        public void drawHighlighted(Canvas c, Highlight[] indices) {
            ...
            for (Highlight high : indices) {
                ...
                path1.addRoundRect(mBarRect, cornerRadiusArr, Path.Direction.CW);
                c.drawPath(path1, mHighlightPaint);
        ...
        protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
            ...
            if (mChart.isDrawBarShadowEnabled()) {
                ...
                for (int i = 0, count = Math.min((int) (Math.ceil((float) (dataSet.getEntryCount()) * phaseX)), dataSet.getEntryCount());
                    ...
                    path2.addRoundRect(mBarShadowRectBuffer, cornerRadiusArr, Path.Direction.CW);
                    c.drawPath(path2, mShadowPaint);
//                    c.drawRoundRect(mBarShadowRectBuffer, mRadius, mRadius, mShadowPaint);
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
                ...
                path3.addRoundRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                        buffer.buffer[j + 3], cornerRadiusArr, Path.Direction.CW);
                c.drawPath(path3, mRenderPaint);
...
//                        buffer.buffer[j + 3], mRadius, mRadius, mRenderPaint);
                ...
                if (drawBorder) {
                    ...
                    path4.addRoundRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                            buffer.buffer[j + 3], cornerRadiusArr, Path.Direction.CW);
                    c.drawPath(path4, mBarBorderPaint);
//                    c.drawRoundRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
